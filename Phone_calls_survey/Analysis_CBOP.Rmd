...
```{r}
# Proportion invalid offers to valid
CBOP_proportion <- CBOP_phones %>% mutate(poz_grupaZawodu = str_sub(zawod_kod, 1, 1)) %>%
  group_by(poz_grupaZawodu) %>% 
  summarise(sum_wolne = sum(wolne, na.rm = TRUE),
            sum_aktual_wolne = sum(aktual_wolne, na.rm = TRUE),
            proportion = round((sum_wolne - sum_aktual_wolne)/(sum_wolne),2))
```
...
model <- glm(formula = valid_bolean ~ poz_grupaZawodu + czas_dostepnosci_przedzial +
             ...
             data = CBOP_phones,
             family = binomial(),
             subset = poz_grupaZawodu != '6' & war_wojewodztwo != 'podlaskie')
