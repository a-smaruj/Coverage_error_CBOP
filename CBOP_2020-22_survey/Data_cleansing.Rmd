...
create_table <- function(file) {
  ...
  cbop_quarter <- read_process_cbop(file)
  ...
  #second error - job offert not in Poland
  cbop_quarter <- cbop_quarter[war_kraj == "Polska"]
  
  #Stats - second error
  stats_vec_quarter <- append(stats_vec_quarter, sum(cbop_quarter$poz_lWolnychMiejsc))
  
  #third error - incorrect contract type 
...
```{r}
#Table for stats
stats_tab <- data.table(error_type = c('Base value', 'First error', 'Second error', 
                                       'Third error'))
```

```{r}
cbop_vec <- vector(mode = "list", length = 7)
files <- Sys.glob('data\\*.json')

for (i in 1:7) {
  cbop <- create_table(files[i])
  cbop_vec[[i]] <- cbop
...
```{r}
#Table with stats - n
stats_tab[, 'all'] <- rowSums(stats_tab[,2:8])
```
